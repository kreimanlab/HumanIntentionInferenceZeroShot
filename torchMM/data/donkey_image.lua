...
function dataset:__init(args)
  ...
  for line in io.lines(args.data_listbin) do 
    local split = {}
    for k in string.gmatch(line, "%S+") do table.insert(split, k) end
    self.databin:insert(split[1])
  end

  for line in io.lines(args.data_listbackground) do 
    local split = {}
    for k in string.gmatch(line, "%S+") do table.insert(split, k) end
    self.databg:insert(split[1])
  end
  ...
  print('found Mengmi: ' .. #self.databg .. ' backgrounds')
...
function dataset:sample(quantity)
   ...
   for i=1,quantity do
      ...
      local bindata_path = self.data_root .. '/' .. self.databin[idx]

      local data_path = self.data_root .. '/' .. self.data[idx]
      local out = self:trainHook(data_path, bindata_path)
...
function dataset:trainHook(path, binpath)
  collectgarbage()
  ...
  --mengmi start
  --add in random background    
  local idx = torch.random(1, #self.databg)
  local bg_path = self.data_root .. '/' .. self.databg[idx]
  --print (bg_path)
  local ok,bgimg = pcall(image.load, bg_path, 3, 'float')

  if not ok then
     print('warning: failed loading: ' .. bg_path) 
     bgimg = torch.zeros(3, oW, oH)     
  end
  bgimg = image.scale(bgimg, opt.fineSize, opt.fineSize)
  
  local ok,mask = pcall(image.load, binpath, 3, 'float')

  if not ok then
     print('warning: failed loading: ' .. binpath) 
     mask = torch.zeros(3, oW, oH)     
  end
  mask = image.scale(mask, opt.fineSize, opt.fineSize)
  --mask = torch.cat({ mask,mask,mask},1)
  
  input:map2(mask, bgimg, function(input, mask, bgimg) return mask*input + (1-mask)*bgimg end)  
  --end of background insertion
